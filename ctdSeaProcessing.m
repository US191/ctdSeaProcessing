...
if ~ exist(strcat(prefdir, filesep, mfilename,'.mat'), 'file')
    ...
    ConfigFile = load(strcat(prefdir, filesep, mfilename,'.mat'), 'p');
    p = ConfigFile.p;
end
%% Initialization
...
msg = sprintf('Copy CTD files from:\n %s \n to :\n %s \n %s',...
    p.path_outputCTD, p.path_rawCTD, p.path_processCTD);
...
adcpm_infogen = uicontrol(panel_ADCP, 'style', 'edit', ...
    ...
    'tag', 'FILEADCPM_ENTER', ...
    'string', strcat(p.path_outputADCP,p.filename_adcpm), ...
...
uicontrol(panel_ADCP, 'string', 'Select', ...
    ...
    'callback', {@choose_file, 'filename_adcpm'});
...
adcps_infogen = uicontrol(panel_ADCP, 'style', 'edit', ...
    ...
    'tag', 'FILEADCPS_ENTER', ...
    'string', strcat(p.path_outputADCP,p.filename_adcps), ...
...
uicontrol(panel_ADCP, 'string', 'Select', ...
    ...
    'callback', {@choose_file, 'filename_adcps'});

% Option to copy ADCP file
msg = sprintf('Copy ADCP files to :\n %s%s \n %s%s \n and \n %s%s \n %s%s \n %s%s \n %s%s', ...
    p.path_moveADCP, p.newfilename_ADCPM,...
    p.path_moveADCP, p.newfilename_ADCPS,...
    p.path_rawADCP, p.newfilename_ADCPM,...
    p.path_rawADCP, p.newfilename_ADCPS,...
    p.path_processADCP, p.newfilename_ADCPM,...
    p.path_processADCP, p.newfilename_ADCPS);
...
% Select files & set parameters
    function choose_file(hObj, ~, member)
        if strcmp(member, 'config_filename')
            ...
            p.(member) = uigetfile('*.000', 'Select file', strcat(p.path_outputADCP, p.(member)));
        end
...
% Set mission parameter
    function set_mission_para
        set(config_file, 'string', p.config_filename);
        set(num_station, 'string', p.num_station);
        set(name_mission, 'string', p.name_mission);
        set(id_mission, 'string', p.id_mission);
        set(adcpm_infogen, 'string', strcat(p.path_outputADCP, p.filename_adcpm));
        set(adcps_infogen, 'string', strcat(p.path_outputADCP, p.filename_adcps));
    end
